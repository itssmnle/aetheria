  ...
  for (const mod of modNames) {
    try {
      ...
      if (data.hits && data.hits.length > 0) {
        ...
        const match = newContent.match(new RegExp(`id:\\s*'${mod.id}',[\\s\\S]*?author:\\s*'.*?'`));
        if (match) {
           const replacement = match[0].replace(/author:\s*'.*?'/, `author: '${author.replace(/'/g, "\\'")}'`);
           newContent = newContent.replace(match[0], replacement);
        }
        console.log(`Updated ${mod.name} with author: ${author}`);
      }
    } catch (e) {
